...
- **Dividend accounting with DRIP**: dividends are detected from market data for shares held before each ex-date and surface as **pending receipts** — apply them (one by one or all) to credit cash, or flip a per-symbol **DRIP** toggle to reinvest into fractional shares at the ex-date close. Dividends flow into the P&L series, a summary tile, and total P&L; short positions are charged, as at a real broker.
