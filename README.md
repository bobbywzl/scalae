...
- **Highlight toolkit — select text, act on it**: selecting text on any desk surface (briefs, dossiers, readings, evidence, chat replies, notepads) raises a small toolbar — iOS-marker **highlights** in four colors with optional comments (reviewable in one annotations log per ticker), and on notepads a **Visualize** tool: the desk reads the highlighted passage against the whole notebook and draws it as an inline chart. It **asks first what you want to see** — a general lens (trend / comparison / event sequence / mechanism map) and/or your specific ask — then generates the one visual the record itself supports, with a provenance line ("from the record: …") and caveats always attached; when the record can't support the ask it says so instead of inventing numbers. The result parks in the dialog for your **keep or discard** — kept visuals sit beneath the notepad (your words are never edited) and can be removed any time.
...
- **Deep daily research runs** — a five-stage multi-agent pipeline, every run:
  1. *Question framing*: the **question suggestor** decides what the desk must answer **today**, before any searching — worked from the certainty-gap master question and the question-generation method over the board's readings, the due-diligence record and your recent guidance. The framed questions steer every later stage and show in the run banner.
  2. *Breadth sweeps (parallel)*: Google-Search-grounded scouts cover **the focus questions head-on**, the signal bundles, broad company news, **primary sources** (filings, transcripts, IR, regulator documents) and **culture scuttlebutt** (employees, customers, suppliers, trade press) — every sweep steered by the day's questions.
  3. *Gap analysis*: the Claude analyst triages the evidence against the board and commissions targeted follow-up questions where a focus question was left open or evidence is thin, conflicting, or red-flag-adjacent.
  4. *Deep-dive sweeps (parallel)*: a stronger scout model runs the commissioned probes.
  5. *Deep synthesis*: the analyst weighs everything into new **readings** for every signal (level, value, confidence, **per-source citations**), a **morning brief** that leads with disconfirming evidence and answers the day's questions, and an **evidence feed** whose items each carry a **source class & recommendation** (primary / trade press / media report — and why that source is, or isn't, the one to open).
- **Evidence mapping**: every reading stores exactly which sources it draws on. Signal cards show clickable source chips and an accumulating **evidence catalog** (grows across runs); the morning brief's `[n]` citations render as **clickable links** into that run's numbered sources. Evidence-feed items emphasize their source: an evidence-class chip (**primary source / trade press / media report**, per the weighing doctrine) plus the desk's one-line recommendation on whether that source is the one worth opening. The Due diligence ↔ Signals surfaces switch via one segmented pill next to the ticker.
...
- **Full-agency analyst desk**: every desk has an analyst chat. It answers from the board's evidence, takes feedback into tomorrow's research, proposes signals — and on your explicit ask it can approve/dismiss pending proposals, retire active signals, or kick off a research run. Chat is **two-lane**: simple questions are answered in seconds by a value-tier fast lane working from a compact desk snapshot, and the turn escalates automatically to the flagship analyst (full context, desk actions) only when you ask for signal work, approvals, document reads, or fresh synthesis — a fast-lane failure falls through to the flagship, never to an error. The desk zooms to **full screen** (state, approvals and live polling carry over), takes **voice dictation** and can **read replies aloud**, and accepts **attachments** — images and charts, PDFs (filings, broker notes) and text files — which the analyst reads natively as evidence.
...
Interactive latency is governed first by **lane routing**: working-chat turns land on the Sonnet fast lane (compact desk snapshot, no attachments inlined, 45s cap, `CLAUDE_CHAT_FAST_MODEL` / `CLAUDE_CHAT_FAST_EFFORT` to re-pin), which answers simple questions in seconds and sets an `escalate` flag — signal building, approvals, document analysis, steering feedback, or anything beyond the snapshot falls through to the flagship deep lane with the turn's remaining time budget. Within a lane, latency is governed by reasoning **effort** (on current models it is the one lever for adaptive-thinking depth, and thinking spends from the same `max_tokens` as the reply): the deep chat lane runs at `low` (`CLAUDE_CHAT_EFFORT` to raise) and the pairwise compare at `medium` (`CLAUDE_COMPARE_EFFORT`), and the whole turn runs under a hard 240s deadline that fails into the retry path instead of riding to the platform's function kill — so the analyst answers in seconds, and a genuinely hung call surfaces as a clean, retryable error rather than a browser timeout.
