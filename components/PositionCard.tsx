  ...
  return (
    <section className="rounded-2xl bg-card border border-hairline px-5 py-4">
      <div className="flex items-center gap-2 flex-wrap">
        <p className="text-[0.6875rem] uppercase tracking-widest text-muted font-semibold">{t("trade.yourPosition")}</p>
        {position.unrealized != null && (
          <span className={`text-[0.6875rem] font-semibold tabular-nums ${signCls(position.unrealized)}`}>
        ...
        {position.realized !== 0 && (
          <span className={`text-[0.6875rem] tabular-nums ${signCls(position.realized)}`}>
            · {t("trade.realizedAmt", {
        ...
        {position.dividends !== 0 && (
          <span
            className={`text-[0.6875rem] tabular-nums ${signCls(position.dividends)}`}
        ...
        {s && s.qty > 0 && (
          <button
            ...
            className={`rounded-full px-2 py-px text-[0.5625rem] font-semibold uppercase tracking-wider transition-colors disabled:opacity-50 ${
        ...
        <Link
          href="/portfolio"
          className="ml-auto text-[0.6875rem] text-accent hover:opacity-80 transition-opacity"
        >
        ...
        {s && (
          <div>
            <p className="text-sm font-semibold tabular-nums">
              {s.qty < 0 && <span className="text-loss text-[0.625rem] font-bold mr-1">{t("trade.shortChip")}</span>}
            ...
            <p className="text-[0.6875rem] text-muted tabular-nums mt-0.5">
              {s.mark != null ? (
                <>
        ...
        {position.options.map((o) => (
          <div key={o.key}>
            <p className="text-sm font-semibold">
              {o.qty < 0 && <span className="text-warn text-[0.625rem] font-bold mr-1">{t("trade.shortChip")}</span>}
              {Math.abs(o.qty)}× {optionLabel(o)}
              {o.expired && <span className="text-loss text-[0.625rem] font-bold ml-1.5">{t("trade.expiredChip")}</span>}
            </p>
            <p className="text-[0.6875rem] text-muted tabular-nums mt-0.5">
