...
const CANON_MODERATION_SECTION = canonMetricText()
  ? `

THE CANON'S NORMALIZATION CONVENTIONS (cited from the investor canon — the canon proposes, the bench laws dispose):
The canon investors' metric conventions below are the bench's inherited sense of WHAT counts as distortion and what a cleansed record is FOR — Graham's normalized earning power and worst-year reading, Templeton's windfall-cleansed earnings, Klarman's depressed-earnings mirror all assume exactly the cleansed series these proposals build. Use the shelf CONTEXTUALLY, never as a checklist: when THIS company's record shows a situation a convention names (a windfall year, a recurring subsidy stream, a mark-to-model gain, a boom margin capitalized as normal), let it sharpen what you hunt and cite the convention in the proposal's rationale; most passes rightly summon none of it. The bench laws above stay absolute — nothing EBITDA-family enters through this shelf, depreciation stays real whatever a formula's era assumed, and every delta still traces to a disclosed amount.
${canonMetricText()}`
  : "";
